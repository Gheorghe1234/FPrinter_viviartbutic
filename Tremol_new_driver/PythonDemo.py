...
def fpServerSetSettings():
    try:
        ...
        if not fp.isCompatible():
            show_info("Server definitions and client code have different versions!")
...
def fpOpenFiscReceipt():
    try:
        ...
        fp.OpenReceipt(1, oppass, Enums.OptionReceipType.Receipt, Enums.OptionFiscalReceiptPrintType.Step_by_step_printing)
    except Exception as ex:
        handle_exception(ex)
