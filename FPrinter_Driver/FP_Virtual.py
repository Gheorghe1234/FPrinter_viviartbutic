...
from FPrinter_Driver.FP_Datecs_Emulator import *
...
class VIRTUAL_Comm:
    def __init__(self, p_current_path):
        ...
        logger.info(f'Init VIRTUAL_Comm module')
        # self.device = client.Dispatch("Wtdedit.FpAtl.1")
        self.device = Emulator(p_current_path)
        self.msg_res = ''
        self.EJ_OPERATIONS = ''
    ...
    def param_convert_new_protocol(self, p_cmd, p_param):
        ...
        try:
            if p_cmd == 33:
                ...
                # v_step1 = p_param.find('\\t')
                #             # v_step2 = v_step1 + 3
                #             # v_step3 = p_param.find('*', v_step2)
                #             # v_step3 = len(p_param) if v_step3 == -1 else v_step3
                #             # v_step4 = p_param.find('\\t', v_step3 + 1)
                #             # v_step4 = len(p_param) if v_step4 == -1 else v_step4
                #             #
                #             # v_produs_name = p_param[:v_step1]
                #             # v_cota_tva = p_param[v_step1 + 2:v_step2]
                #             # v_price = p_param[v_step2:v_step3]
                #             # v_qnt = p_param[v_step3 + 1:v_step4]
                #             # v_qnt = 1 if v_qnt == '' else v_qnt
                #             # v_um = p_param[v_step4 + 2:]
                #             # v_um = 'buc.' if v_um == '' else v_um
                ...
                v_result = f'{v_produs_name}\t{v_cota_tva}\t{v_price}\t{v_qnt}\t0\t0\t{v_departament}\t{v_um}\t'
