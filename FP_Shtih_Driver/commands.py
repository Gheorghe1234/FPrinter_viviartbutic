...
import time
from . import misc, excepts_shtrih


class Commands:
    def __init__(self, p_fp):
        self.WAIT_TIME = 0.01
        self.DEFAULT_MAX_LENGTH = 40
        self.admin_password = 30
        self.protocol = p_fp
        self.password = 30
    ...
    def title_doc(self):
        ...
        return self.protocol.command(
            0x18,
            self.password,
            misc.prepare_string2('', 30),
            misc.CAST_SIZE['2'](1),
        )
    title_doc.cmd = 0x18

    def finis_document(self):
        """
    ...
    def sale(self, item, department_num=0, tax1=0, tax2=0, tax3=0, tax4=0):
        ...
        except excepts_shtrih.Error as exc:
            raise excepts_shtrih.ItemSaleError(exc)
    sale.cmd = 0x80
    ...
    def close_check(self,
        ...
        except excepts_shtrih.ProtocolError as exc:
            raise excepts_shtrih.CloseCheckError(exc)
    close_check.cmd = 0x85

    def open_non_fiscal_document(self):
        """
          Открыть нефискальный документ
        """

        self.title_doc()
        return "open_non_fiscal_document - {'Error code': 0, 'Operator serial number': 30}"
    # open_non_fiscal_document.cmd = 0xE2

    def close_non_fiscal_document(self):
        """
          Открыть нефискальный документ
        """
        ...
        self.finis_document()
        return "open_non_fiscal_document - {'Error code': 0, 'Operator serial number': 30}"
    # close_non_fiscal_document.cmd = 0xE3
    ...
    def open_check(self, check_type, p_password=None):
        """
        Открыть чек. p_password
        """

        p_password = self.password if p_password is None else p_password
        self.wait_printing()
        try:
            return self.protocol.command(
                0x8D,
            ...
            raise excepts_shtrih.OpenCheckError(exc)
    open_check.cmd = 0x8D
    ...
    title_doc.depends = (wait_printing, )
    finis_document.depends = (wait_printing, )
    ...
    # open_non_fiscal_document().depends = (wait_printing, )
    # close_non_fiscal_document().depends = (wait_printing, )
