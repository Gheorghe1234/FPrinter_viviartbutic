...
from . import excepts_shtrih , misc
...
class Protocol(object):
        ...
        if not self.connected:
            ...
            if not self.serial.isOpen():
                try:
                    ...
                    raise excepts_shtrih.NoConnectionError(
    ...
    def init(self):
        ...
        try:
            ...
            raise excepts_shtrih.ProtocolError(u'Не удалось записать байт в ККМ')
        except serial.SerialException as exc:
            self.serial.flushInput()
            raise excepts_shtrih.ProtocolError(unicode(exc))
    ...
    def handle_response(self):
        ...
        for _ in xrange(self.MAX_ATTEMPTS):
            ...
            if misc.lrc(misc.bytearray_concat(length, payload)) == _lrc:
                ...
                byte = self.serial.read()
                if byte != ACK:
                    raise excepts_shtrih.UnexpectedResponseError(u'Получен байт 0x{:02X}, ожидался ACK'.format(ord(byte)))
        else:
            raise excepts_shtrih.NoConnectionError()
    ...
    def handle_payload(self, payload):
        ...
        try:
            ...
            raise excepts_shtrih.UnexpectedResponseError(u'Не удалось получить байт(ы) команды из ответа')
        ...
        if handler:
            ...
            if error != 0:
                raise excepts_shtrih.Error(cmd, error, fs=self.fs)
    ...
    def check(self, count=1, quiet=False):
        ...
        if self.serial is None:
            raise excepts_shtrih.ProtocolError(u'Необходимо вначале выполнить метод connect()')
        ...
        for _ in xrange(count):
            try:
                yield self.init()
            except excepts_shtrih.NoConnectionError:
                if quiet:
                    yield False
                else:
                    raise
